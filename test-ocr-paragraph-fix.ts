...
async function testOCRParagraphFix() {
  ...
  try {
    ...
    console.log(`   • Average confidence: ${Math.round(result.confidence)}%`);
    console.log(
      `   • Text length: ${result.extractedText.length.toLocaleString()} characters`,
    );
    console.log(
      `   • Paragraphs detected: ${result.detectedStructure.paragraphs.length}`,
    );
    ...
    writeFileSync(textOutputPath, result.extractedText, "utf8");
