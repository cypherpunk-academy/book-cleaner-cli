...
export class FileFormatDetector {
    ...
    async detectFormat(fileInfo: FileInfo): Promise<FileFormatResult> {
        const detectorLogger = this.logger.getTextExtractionLogger(LOG_COMPONENTS.FILE_HANDLER);
    ...
    private isLikelyTextFile(buffer: Buffer): boolean {
        ...
        return printableCount / totalChecked > VALIDATION_THRESHOLDS.TEXT_PRINTABLE_RATIO;
    }
     ...
     */
    private validateFileSize(fileSize: number, format: SupportedFormat): ValidationResult {
        const issues: ValidationIssue[] = [];
        let isValid = true;

        const limit = FILE_SIZE_LIMITS[format.toUpperCase() as keyof typeof FILE_SIZE_LIMITS];
        ...
        try {
            switch (format) {
                ...
                default:
                    return {
                        isValid: false,
                        issues: [{ type: 'error', message: `Unsupported format: ${format}` }],
                        metadata: {},
                    };
            }
        } catch (error) {
    ...
    private async validatePDF(filePath: string): Promise<PDFValidationResult> {
        ...
        try {
            ...
            } else if (!hasEmbeddedText || textLength < VALIDATION_THRESHOLDS.MIN_TEXT_LENGTH) {
                if (hasImages) {
                    ...
                    issues.push({
                        ...
                        details: {
                            ...
                            hasEmbeddedText,
                            estimatedImageCount,
                            imagePages: imagePages.length > 0 ? imagePages.slice(0, 5) : [],
                        },
                    });
                } else {
                    contentType = PDF_CONTENT_TYPES.EMPTY;
            ...
            return {
                ...
                issues,
                metadata,
                contentType: contentType as 'text_based' | 'image_based' | 'hybrid' | 'empty',
