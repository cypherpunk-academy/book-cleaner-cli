...
export interface QualityIssue {
    type: 'spelling_error' | 'ocr_debris' | 'broken_word' | 'weird_character' | 'formatting_issue';
    position: number;
    length: number;
...
export interface QualityImprovement {
    type: 'spelling_correction' | 'debris_removal' | 'word_reconstruction' | 'character_cleaning';
    originalText: string;
    improvedText: string;
    position: number;
    confidence: number;
...
export class TextQualityAnalyzer {
    ...
    private detectOCRDebris(text: string): QualityIssue[] {
        ...
        for (const pattern of debrisPatterns) {
    ...
    private detectBrokenWords(text: string): QualityIssue[] {
        ...
        let match: RegExpExecArray | null = brokenWordPattern.exec(text);
        while (match !== null) {
            issues.push({
                type: 'broken_word',
                position: match.index,
            ...
            match = brokenWordPattern.exec(text);
        }
     ...
     */
    private detectSpellingErrors(_text: string, _options: QualityAnalysisOptions): QualityIssue[] {
        const issues: QualityIssue[] = [];
