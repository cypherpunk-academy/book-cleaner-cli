...
export class TextQualityEnhancementExecutionSummary {
    ...
    markStarted(): void {
        ...
        const summaryLogger = this.logger.getTextExtractionLogger(LOG_COMPONENTS.PIPELINE_MANAGER);
    ...
    recordInputMetrics(text: string, sourceFiles: string[]): void {
        this.summary.inputMetrics = {
            ...
            sentenceCount: text.split(/[.!?]+/).filter((sentence) => sentence.trim().length > 0)
                .length,
     ...
     */
    recordOutputMetrics(originalText: string, enhancedText: string, outputFiles: string[]): void {
        const qualityImprovement =
            this.summary.qualityAnalysis.overallQuality > 0
                ? this.summary.validation.overallScore - this.summary.qualityAnalysis.overallQuality
                : 0;
